...
class MyCommunity(Community, PeerObserver):
    ...
    async def generate_transaction(self) -> None:
        ...
        signature = self.crypto.create_signature(self.my_peer.key, nonce.to_bytes(8))
    ...
    def on_transaction_message(self, peer: Peer, payload: TransactionMessage):
        ...
        pub_key = self.crypto.key_from_public_bin(bytes.fromhex(payload.public_key))
    ...
    def started(self) -> None:
        ...
        self.add_message_handler(TransactionMessage, self.on_transaction_message)

        self.register_task('generate_transaction', self.generate_transaction, interval=5.0, delay=0)

...
from messages import (
    ...
    MempoolRequestPayload,
    MempoolResponsePayload,
    CommitPayload,
    RevealPayload,
)


class LotteryCommunity(Community):
    """Community for the lottery blockchain network"""

    community_id = b"\x01LotteryNetwork"

    def __init__(
        self, my_peer, endpoint, network, db_path="./lottery_db", max_peers=10
    ):
        ...
        self.register_task(
            "lottery_cycle", self.lottery_cycle, interval=30.0
        )  # 30 seconds = 1 day in simulation

        # For testing: place random bets
        self.register_task(
            "place_random_bet",
            self.place_random_bet,
            interval=random.uniform(3.0, 10.0),
        )
    ...
    def started(self) -> None:
        ...
        super().started()
        print(
            f"Lottery peer {self.my_peer.mid.hex()[:8]} started with stake {self.stake}"
        )
    ...
    def place_random_bet(self) -> None:
        ...
        transaction = {
            "id": hashlib.sha256(
                f"{time.time()}-{self.my_peer.mid.hex()}".encode()
            ).hexdigest(),
    ...
    def on_block(self, peer: Peer, payload: BlockPayload) -> None:
        ...
        if block.index == len(self.blockchain.chain):
            ...
            if block.previous_hash == self.blockchain.get_latest_block().hash:
                ...
                if results["winners"]:
                    print(
                        f"Lottery results: winning number {results['winning_number']}, {len(results['winners'])} winners"
                    )
                else:
                    print(
                        f"Lottery results: winning number {results['winning_number']}, no winners"
                    )
    ...
    def on_chain_request(self, peer: Peer, payload: ChainRequestPayload) -> None:
        ...
        blocks_to_send = [
            block.to_dict() for block in self.blockchain.chain[latest_block_index + 1 :]
        ]
        if blocks_to_send:
    ...
    def on_chain_response(self, peer: Peer, payload: ChainResponsePayload) -> None:
        ...
        if (
            blocks[0].index == len(self.blockchain.chain)
            and blocks[0].previous_hash == self.blockchain.get_latest_block().hash
        ):
            # Add all blocks
            for block in blocks:
                self.blockchain.chain.append(block)
    ...
    def start_reveal_phase(self) -> None:
        ...
        payload = RevealPayload(
            self.my_peer.mid.hex(), self.my_random_value, self.my_nonce
        )
    ...
    def on_reveal(self, peer: Peer, payload: RevealPayload) -> None:
        ...
        if peer_id in self.commits:
            ...
            if verify_hash == commit_hash:
                # Store valid reveal
                self.reveals[peer_id] = {"value": random_value, "nonce": nonce}
        ...
        if selected_validator == self.my_peer.mid.hex():
            ...
            if results["winners"]:
                ...
                for winner in results["winners"]:
                    print(
                        f"  Winner: {winner['sender'][:8]}, bet: {winner['amount']}, reward: {winner['reward']:.2f}"
                    )
            else:
                print(
                    f"Lottery results: winning number {results['winning_number']}, no winners"
